...
from utils.dedupe import merge_subdomains, normalize_subdomain, is_valid_subdomain
...
async def _run_subfinder(target: str, config: ReconConfig, limiter) -> list[Subdomain]:
    ...
    try:
        ...
        stdout, stderr = await asyncio.wait_for(proc.communicate(), timeout=config.checkpoint_file.parent.stat().st_size if config.checkpoint_file else 120)
...
async def _run_amass(target: str, config: ReconConfig, limiter) -> list[Subdomain]:
    ...
    try:
        proc = await asyncio.create_subprocess_exec(
            "amass", "enum", "-d", target, "-json", "-silent",
            stdout=asyncio.subprocess.PIPE,
            stderr=asyncio.subprocess.PIPE,
        )
